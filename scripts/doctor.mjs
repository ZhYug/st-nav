...
console.log('🔎 ST Nav v1.1.4 Pages Advanced Mode 检查\n');
...
if (existsSync('wrangler.toml')) {
  ...
  if (!/ST_NAV_VERSION\s*=\s*["']1\.1\.4["']/.test(config)) fail('wrangler.toml 版本号不是 1.1.4');
  console.log('✓ Pages + D1 配置正确');
}

if (existsSync('package.json')) {
  const pkg = JSON.parse(readFileSync('package.json', 'utf8'));
  if (pkg.version !== '1.1.4') fail('package.json 版本号不是 1.1.4');
...
if (existsSync('public/_worker.js')) {
  ...
  if (!worker.includes('const VERSION = "1.1.4"')) fail('Worker 版本号不是 1.1.4');
